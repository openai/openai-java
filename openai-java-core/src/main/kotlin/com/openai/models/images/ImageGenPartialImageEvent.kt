...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        private var type: JsonValue = JsonValue.from("image_generation.partial_image")
        private var additionalProperties: MutableMap<String, JsonValue> = mutableMapOf()

        @JvmSynthetic
        internal fun from(imageGenPartialImageEvent: ImageGenPartialImageEvent) = apply {
            b64Json = imageGenPartialImageEvent.b64Json
            background = imageGenPartialImageEvent.background
            createdAt = imageGenPartialImageEvent.createdAt
            outputFormat = imageGenPartialImageEvent.outputFormat
            partialImageIndex = imageGenPartialImageEvent.partialImageIndex
            quality = imageGenPartialImageEvent.quality
            size = imageGenPartialImageEvent.size
            type = imageGenPartialImageEvent.type
            additionalProperties = imageGenPartialImageEvent.additionalProperties.toMutableMap()
        }
         ...
         */
        fun build(): ImageGenPartialImageEvent =
    ...
    fun validate(): ImageGenPartialImageEvent = apply {
        if (validated) {
            return@apply
        }
        ...
        size().validate()
        _type().let {
            if (it != JsonValue.from("image_generation.partial_image")) {
    ...
    internal fun validity(): Int =
        (if (b64Json.asKnown().isPresent) 1 else 0) +
            ...
            type.let { if (it == JsonValue.from("image_generation.partial_image")) 1 else 0 }
    ...
    override fun toString() =
        "ImageGenPartialImageEvent{b64Json=$b64Json, background=$background, createdAt=$createdAt, outputFormat=$outputFormat, partialImageIndex=$partialImageIndex, quality=$quality, size=$size, type=$type, additionalProperties=$additionalProperties}"
}
