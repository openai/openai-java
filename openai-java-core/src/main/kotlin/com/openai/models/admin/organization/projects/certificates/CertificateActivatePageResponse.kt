...
private constructor(
     ...
     * ```java
     * JsonValue.from("organization.project.certificate.activation")
     * ```
     *
    ...
    fun validate(): CertificateActivatePageResponse = apply {
        ...
        _object_().let {
            if (it != JsonValue.from("organization.project.certificate.activation")) {
    ...
    internal fun validity(): Int =
        (data.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
            object_.let {
                if (it == JsonValue.from("organization.project.certificate.activation")) 1 else 0
            }
