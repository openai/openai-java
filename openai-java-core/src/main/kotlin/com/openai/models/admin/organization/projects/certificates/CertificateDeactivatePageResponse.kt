...
private constructor(
     ...
     * ```java
     * JsonValue.from("organization.project.certificate.deactivation")
     * ```
     *
    ...
    fun validate(): CertificateDeactivatePageResponse = apply {
        ...
        _object_().let {
            if (it != JsonValue.from("organization.project.certificate.deactivation")) {
    ...
    internal fun validity(): Int =
        (data.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
            object_.let {
                if (it == JsonValue.from("organization.project.certificate.deactivation")) 1 else 0
            }
