...
@JsonDeserialize(using = BetaMcpToolCallError.Deserializer::class)
@JsonSerialize(using = BetaMcpToolCallError.Serializer::class)
class BetaMcpToolCallError
private constructor(
    private val mcpProtocol: McpProtocolError? = null,
    private val mcpToolExecution: McpToolExecutionError? = null,
    private val http: HttpError? = null,
    private val _json: JsonValue? = null,
) {

    fun mcpProtocol(): Optional<McpProtocolError> = Optional.ofNullable(mcpProtocol)

    fun mcpToolExecution(): Optional<McpToolExecutionError> = Optional.ofNullable(mcpToolExecution)

    fun http(): Optional<HttpError> = Optional.ofNullable(http)

    fun isMcpProtocol(): Boolean = mcpProtocol != null

    fun isMcpToolExecution(): Boolean = mcpToolExecution != null

    fun isHttp(): Boolean = http != null

    fun asMcpProtocol(): McpProtocolError = mcpProtocol.getOrThrow("mcpProtocol")

    fun asMcpToolExecution(): McpToolExecutionError =
        mcpToolExecution.getOrThrow("mcpToolExecution")
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = betaMcpToolCallError.accept(new BetaMcpToolCallError.Visitor<Optional<String>>() {
     *     @Override
     *     public Optional<String> visitMcpProtocol(McpProtocolError mcpProtocol) {
     *         return Optional.of(mcpProtocol.toString());
     *     }
     *
     *     // ...
     *
    ...
    fun <T> accept(visitor: Visitor<T>): T =
        when {
            mcpProtocol != null -> visitor.visitMcpProtocol(mcpProtocol)
            mcpToolExecution != null -> visitor.visitMcpToolExecution(mcpToolExecution)
     ...
     *   expected type.
     */
    fun validate(): BetaMcpToolCallError = apply {
        if (validated) {
            return@apply
        }
        ...
        accept(
            object : Visitor<Unit> {
                override fun visitMcpProtocol(mcpProtocol: McpProtocolError) {
                    mcpProtocol.validate()
                }

                override fun visitMcpToolExecution(mcpToolExecution: McpToolExecutionError) {
                    mcpToolExecution.validate()
                }
    ...
    internal fun validity(): Int =
        accept(
            object : Visitor<Int> {
                override fun visitMcpProtocol(mcpProtocol: McpProtocolError) =
                    mcpProtocol.validity()

                override fun visitMcpToolExecution(mcpToolExecution: McpToolExecutionError) =
                    mcpToolExecution.validity()
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is BetaMcpToolCallError &&
            mcpProtocol == other.mcpProtocol &&
            mcpToolExecution == other.mcpToolExecution &&
            http == other.http
    }

    override fun hashCode(): Int = Objects.hash(mcpProtocol, mcpToolExecution, http)

    override fun toString(): String =
        when {
            mcpProtocol != null -> "BetaMcpToolCallError{mcpProtocol=$mcpProtocol}"
            mcpToolExecution != null -> "BetaMcpToolCallError{mcpToolExecution=$mcpToolExecution}"
            http != null -> "BetaMcpToolCallError{http=$http}"
            _json != null -> "BetaMcpToolCallError{_unknown=$_json}"
            else -> throw IllegalStateException("Invalid BetaMcpToolCallError")
        }

    companion object {

        @JvmStatic
        fun ofMcpProtocol(mcpProtocol: McpProtocolError) =
            BetaMcpToolCallError(mcpProtocol = mcpProtocol)

        @JvmStatic
        fun ofMcpToolExecution(mcpToolExecution: McpToolExecutionError) =
            BetaMcpToolCallError(mcpToolExecution = mcpToolExecution)

        @JvmStatic fun ofHttp(http: HttpError) = BetaMcpToolCallError(http = http)
    }

    /**
     * An interface that defines how to map each variant of [BetaMcpToolCallError] to a value of
     * type [T].
     */
    interface Visitor<out T> {

        fun visitMcpProtocol(mcpProtocol: McpProtocolError): T =
            unknown(JsonValue.from(mcpProtocol))

        fun visitMcpToolExecution(mcpToolExecution: McpToolExecutionError): T =
            unknown(JsonValue.from(mcpToolExecution))

        fun visitHttp(http: HttpError): T = unknown(JsonValue.from(http))

        /**
         * Maps an unknown variant of [BetaMcpToolCallError] to a value of type [T].
         *
         * An instance of [BetaMcpToolCallError] can contain an unknown variant if it was
    ...
    internal class Deserializer :
        BaseDeserializer<BetaMcpToolCallError>(BetaMcpToolCallError::class) {

        override fun ObjectCodec.deserialize(node: JsonNode): BetaMcpToolCallError {
            ...
            when (type) {
                "mcp_protocol_error" -> {
                    return tryDeserialize(node, jacksonTypeRef<McpProtocolError>())?.let {
                        BetaMcpToolCallError(mcpProtocol = it, _json = json)
                    } ?: BetaMcpToolCallError(_json = json)
                }
                "mcp_tool_execution_error" -> {
                    return tryDeserialize(node, jacksonTypeRef<McpToolExecutionError>())?.let {
                        BetaMcpToolCallError(mcpToolExecution = it, _json = json)
                    } ?: BetaMcpToolCallError(_json = json)
                }
                "http_error" -> {
                    return tryDeserialize(node, jacksonTypeRef<HttpError>())?.let {
                        BetaMcpToolCallError(http = it, _json = json)
                    } ?: BetaMcpToolCallError(_json = json)
                }
            }

            return BetaMcpToolCallError(_json = json)
        }
    }

    internal class Serializer : BaseSerializer<BetaMcpToolCallError>(BetaMcpToolCallError::class) {

        override fun serialize(
            value: BetaMcpToolCallError,
            generator: JsonGenerator,
            provider: SerializerProvider,
        ) {
            when {
                value.mcpProtocol != null -> generator.writeObject(value.mcpProtocol)
                value.mcpToolExecution != null -> generator.writeObject(value.mcpToolExecution)
