...
@JsonDeserialize(using = AgentToolParam.Deserializer::class)
@JsonSerialize(using = AgentToolParam.Serializer::class)
...
private constructor(
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = agentToolParam.accept(new AgentToolParam.Visitor<Optional<String>>() {
     *     @Override
     ...
     *   expected type.
     */
    fun validate(): AgentToolParam = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is AgentToolParam &&
    ...
    override fun toString(): String =
        when {
            function != null -> "AgentToolParam{function=$function}"
            toolSearch != null -> "AgentToolParam{toolSearch=$toolSearch}"
            programmaticToolCalling != null ->
                "AgentToolParam{programmaticToolCalling=$programmaticToolCalling}"
            mcp != null -> "AgentToolParam{mcp=$mcp}"
            webSearch != null -> "AgentToolParam{webSearch=$webSearch}"
            _json != null -> "AgentToolParam{_unknown=$_json}"
            else -> throw IllegalStateException("Invalid AgentToolParam")
        }

    companion object {

        /** A function defined by the application. */
        @JvmStatic fun ofFunction(function: Function) = AgentToolParam(function = function)

        /** Discovers deferred function tools and loads them into the model context. */
        @JvmStatic
        fun ofToolSearch() =
            AgentToolParam(toolSearch = JsonValue.from(mapOf("type" to "tool_search")))
        ...
        /** Tools provided by a remote MCP server. */
        @JvmStatic fun ofMcp(mcp: Mcp) = AgentToolParam(mcp = mcp)

        /** Web search. */
        @JvmStatic fun ofWebSearch(webSearch: WebSearch) = AgentToolParam(webSearch = webSearch)
    }

    /**
     * An interface that defines how to map each variant of [AgentToolParam] to a value of type [T].
     */
    interface Visitor<out T> {
         ...
         *
         * An instance of [AgentToolParam] can contain an unknown variant if it was deserialized
        ...
        fun unknown(json: JsonValue?): T {
            throw OpenAIInvalidDataException("Unknown AgentToolParam: $json")
        }
    }

    internal class Deserializer : BaseDeserializer<AgentToolParam>(AgentToolParam::class) {

        override fun ObjectCodec.deserialize(node: JsonNode): AgentToolParam {
            ...
            when (type) {
                "function" -> {
                    return tryDeserialize(node, jacksonTypeRef<Function>())?.let {
                        AgentToolParam(function = it, _json = json)
                    } ?: AgentToolParam(_json = json)
                }
                "tool_search" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                        ?.let { AgentToolParam(toolSearch = it, _json = json) }
                        ?.takeIf { it.isValid() } ?: AgentToolParam(_json = json)
                }
                "programmatic_tool_calling" -> {
                    return tryDeserialize(node, jacksonTypeRef<ProgrammaticToolCalling>())?.let {
                        AgentToolParam(programmaticToolCalling = it, _json = json)
                    } ?: AgentToolParam(_json = json)
                }
                "mcp" -> {
                    return tryDeserialize(node, jacksonTypeRef<Mcp>())?.let {
                        AgentToolParam(mcp = it, _json = json)
                    } ?: AgentToolParam(_json = json)
                }
                "web_search" -> {
                    return tryDeserialize(node, jacksonTypeRef<WebSearch>())?.let {
                        AgentToolParam(webSearch = it, _json = json)
                    } ?: AgentToolParam(_json = json)
                }
            }

            return AgentToolParam(_json = json)
        }
    }

    internal class Serializer : BaseSerializer<AgentToolParam>(AgentToolParam::class) {

        override fun serialize(
            value: AgentToolParam,
    ...
    private constructor(
        ...
        private val transport: JsonField<McpTransportParam>,
        private val type: JsonValue,
        ...
        private constructor(
            ...
            @ExcludeMissing
            transport: JsonField<McpTransportParam> = JsonMissing.of(),
            @JsonProperty("type") @ExcludeMissing type: JsonValue = JsonMissing.of(),
         ...
         *   unexpectedly missing or null (e.g. if the server responded with an unexpected value).
         */
        fun transport(): McpTransportParam = transport.getRequired("transport")
        ...
        @JsonProperty("transport")
        @ExcludeMissing
        fun _transport(): JsonField<McpTransportParam> = transport
        ...
        class Builder internal constructor() {
            ...
            private var transport: JsonField<McpTransportParam>? = null
            ...
            /** The transport used to connect to the MCP server. */
            fun transport(transport: McpTransportParam) = transport(JsonField.of(transport))

            /**
             * Sets [Builder.transport] to an arbitrary JSON value.
             *
             * You should usually call [Builder.transport] with a well-typed [McpTransportParam]
             * value instead. This method is primarily for setting the field to an undocumented or
             * not yet supported value.
             */
            fun transport(transport: JsonField<McpTransportParam>) = apply {
                this.transport = transport
            }

            /** Alias for calling [transport] with `McpTransportParam.ofHttp(http)`. */
            fun transport(http: McpTransportParam.Http) = transport(McpTransportParam.ofHttp(http))
            ...
            fun httpTransport(serverUrl: String) =
                transport(McpTransportParam.Http.builder().serverUrl(serverUrl).build())

            /** Alias for calling [transport] with `McpTransportParam.ofStdio(stdio)`. */
            fun transport(stdio: McpTransportParam.Stdio) =
                transport(McpTransportParam.ofStdio(stdio))
            ...
            /**
             * The attached vault credential used to authenticate this MCP server. Optional when
