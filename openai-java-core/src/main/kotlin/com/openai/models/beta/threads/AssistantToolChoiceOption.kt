...
private constructor(
    ...
    interface Visitor<out T> {
        ...
        fun unknown(json: JsonValue?): T {
            throw OpenAIInvalidDataException("Unknown AssistantToolChoiceOption")
        }
    }
    ...
    internal class Deserializer :
        ...
        override fun ObjectCodec.deserialize(node: JsonNode): AssistantToolChoiceOption {
            ...
            val bestMatches =
                sequenceOf(
                        tryDeserialize(node, jacksonTypeRef<Auto>())?.let {
                            AssistantToolChoiceOption(auto = it, _json = json)
                        },
                        tryDeserialize(node, jacksonTypeRef<AssistantToolChoice>())?.let {
                            AssistantToolChoiceOption(assistantToolChoice = it, _json = json)
                        },
                    )
                    .filterNotNull()
            ...
            return when (bestMatches.size) {
                ...
                0 -> AssistantToolChoiceOption(_json = json)
