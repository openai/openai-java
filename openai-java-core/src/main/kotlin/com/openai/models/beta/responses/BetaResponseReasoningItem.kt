...
private constructor(
    ...
    ) : this(id, summary, type, agent, content, encryptedContent, status, mutableMapOf())
    ...
    /** A builder for [BetaResponseReasoningItem]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(betaResponseReasoningItem: BetaResponseReasoningItem) = apply {
            id = betaResponseReasoningItem.id
            summary = betaResponseReasoningItem.summary.map { it.toMutableList() }
            type = betaResponseReasoningItem.type
            agent = betaResponseReasoningItem.agent
            content = betaResponseReasoningItem.content.map { it.toMutableList() }
            encryptedContent = betaResponseReasoningItem.encryptedContent
            status = betaResponseReasoningItem.status
            additionalProperties = betaResponseReasoningItem.additionalProperties.toMutableMap()
        }
         ...
         * When streaming, use the completed reasoning item and its `encrypted_content` from the
         * `response.output_item.done` event in subsequent requests. The `encrypted_content` in
         * `response.output_item.added` may be incomplete. This is especially important when `store`
         * is `false` or when using Zero Data Retention.
         */
         ...
         */
        fun build(): BetaResponseReasoningItem =
     ...
     *   expected type.
     */
    fun validate(): BetaResponseReasoningItem = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is BetaResponseReasoningItem &&
            id == other.id &&
    ...
    override fun toString() =
        "BetaResponseReasoningItem{id=$id, summary=$summary, type=$type, agent=$agent, content=$content, encryptedContent=$encryptedContent, status=$status, additionalProperties=$additionalProperties}"
}
