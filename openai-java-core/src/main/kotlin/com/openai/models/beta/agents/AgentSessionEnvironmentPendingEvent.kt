...
private constructor(
     ...
     * ```java
     * JsonValue.from("agent.session.environment.pending")
     * ```
     *
    ...
    /** A builder for [AgentSessionEnvironmentPendingEvent]. */
    class Builder internal constructor() {
        ...
        private var type: JsonValue = JsonValue.from("agent.session.environment.pending")
        private var additionalProperties: MutableMap<String, JsonValue> = mutableMapOf()

        @JvmSynthetic
        internal fun from(
            agentSessionEnvironmentPendingEvent: AgentSessionEnvironmentPendingEvent
        ) = apply {
            environment = agentSessionEnvironmentPendingEvent.environment
            eventId = agentSessionEnvironmentPendingEvent.eventId
            sessionId = agentSessionEnvironmentPendingEvent.sessionId
            turnId = agentSessionEnvironmentPendingEvent.turnId
            type = agentSessionEnvironmentPendingEvent.type
            additionalProperties =
                agentSessionEnvironmentPendingEvent.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [AgentSessionEnvironmentPendingEvent].
         *
         ...
         */
        fun build(): AgentSessionEnvironmentPendingEvent =
            AgentSessionEnvironmentPendingEvent(
     ...
     *   expected type.
     */
    fun validate(): AgentSessionEnvironmentPendingEvent = apply {
        if (validated) {
            return@apply
        }
        ...
        turnId()
        _type().let {
            if (it != JsonValue.from("agent.session.environment.pending")) {
    ...
    internal fun validity(): Int =
        (environment.asKnown().getOrNull()?.validity() ?: 0) +
            ...
            type.let { if (it == JsonValue.from("agent.session.environment.pending")) 1 else 0 }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is AgentSessionEnvironmentPendingEvent &&
    ...
    override fun toString() =
        "AgentSessionEnvironmentPendingEvent{environment=$environment, eventId=$eventId, sessionId=$sessionId, turnId=$turnId, type=$type, additionalProperties=$additionalProperties}"
}
