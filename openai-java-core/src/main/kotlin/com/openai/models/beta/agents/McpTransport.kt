...
@JsonDeserialize(using = McpTransport.Deserializer::class)
@JsonSerialize(using = McpTransport.Serializer::class)
class McpTransport
private constructor(
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = mcpTransport.accept(new McpTransport.Visitor<Optional<String>>() {
     *     @Override
     ...
     *   expected type.
     */
    fun validate(): McpTransport = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is McpTransport && http == other.http && stdio == other.stdio
    }
    ...
    override fun toString(): String =
        when {
            http != null -> "McpTransport{http=$http}"
            stdio != null -> "McpTransport{stdio=$stdio}"
            _json != null -> "McpTransport{_unknown=$_json}"
            else -> throw IllegalStateException("Invalid McpTransport")
        }

    companion object {

        /** Connects to an MCP server over HTTP. */
        @JvmStatic fun ofHttp(http: Http) = McpTransport(http = http)

        /** Starts an MCP server as a local process. */
        @JvmStatic fun ofStdio(stdio: Stdio) = McpTransport(stdio = stdio)
    }

    /**
     * An interface that defines how to map each variant of [McpTransport] to a value of type [T].
     */
    interface Visitor<out T> {
        ...
        /**
         * Maps an unknown variant of [McpTransport] to a value of type [T].
         *
         * An instance of [McpTransport] can contain an unknown variant if it was deserialized from
    ...
    internal class Deserializer : BaseDeserializer<McpTransport>(McpTransport::class) {

        override fun ObjectCodec.deserialize(node: JsonNode): McpTransport {
            ...
            when (type) {
                "http" -> {
                    return tryDeserialize(node, jacksonTypeRef<Http>())?.let {
                        McpTransport(http = it, _json = json)
                    } ?: McpTransport(_json = json)
                }
                "stdio" -> {
                    return tryDeserialize(node, jacksonTypeRef<Stdio>())?.let {
                        McpTransport(stdio = it, _json = json)
                    } ?: McpTransport(_json = json)
                }
            }

            return McpTransport(_json = json)
        }
    }

    internal class Serializer : BaseSerializer<McpTransport>(McpTransport::class) {
    ...
    private constructor(
        ...
        ) : this(serverUrl, type, mutableMapOf())
        ...
        private val hashCode: Int by lazy { Objects.hash(serverUrl, type, additionalProperties) }

        override fun hashCode(): Int = hashCode

        override fun toString() =
            "Http{serverUrl=$serverUrl, type=$type, additionalProperties=$additionalProperties}"
    }
