 ...
 * to the environment.
 */
@JsonDeserialize(using = CredentialNetworking.Deserializer::class)
@JsonSerialize(using = CredentialNetworking.Serializer::class)
class CredentialNetworking
private constructor(
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = credentialNetworking.accept(new CredentialNetworking.Visitor<Optional<String>>() {
     *     @Override
     ...
     *   expected type.
     */
    fun validate(): CredentialNetworking = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is CredentialNetworking &&
    ...
    override fun toString(): String =
        when {
            unrestricted != null -> "CredentialNetworking{unrestricted=$unrestricted}"
            limited != null -> "CredentialNetworking{limited=$limited}"
            _json != null -> "CredentialNetworking{_unknown=$_json}"
            else -> throw IllegalStateException("Invalid CredentialNetworking")
        }
    ...
    companion object {
        ...
        @JvmStatic
        fun ofUnrestricted() =
            CredentialNetworking(unrestricted = JsonValue.from(mapOf("type" to "unrestricted")))

        /**
         * Allows substitution only for the listed hosts. The environment network policy must also
         * allow these hosts.
         */
        @JvmStatic fun ofLimited(limited: Limited) = CredentialNetworking(limited = limited)
    }

    /**
     * An interface that defines how to map each variant of [CredentialNetworking] to a value of
     * type [T].
     */
    interface Visitor<out T> {
         ...
         *
         * An instance of [CredentialNetworking] can contain an unknown variant if it was
        ...
        fun unknown(json: JsonValue?): T {
            throw OpenAIInvalidDataException("Unknown CredentialNetworking: $json")
        }
    }

    internal class Deserializer :
        BaseDeserializer<CredentialNetworking>(CredentialNetworking::class) {

        override fun ObjectCodec.deserialize(node: JsonNode): CredentialNetworking {
            ...
            when (type) {
                "unrestricted" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                        ?.let { CredentialNetworking(unrestricted = it, _json = json) }
                        ?.takeIf { it.isValid() } ?: CredentialNetworking(_json = json)
                }
                "limited" -> {
                    return tryDeserialize(node, jacksonTypeRef<Limited>())?.let {
                        CredentialNetworking(limited = it, _json = json)
                    } ?: CredentialNetworking(_json = json)
                }
            }

            return CredentialNetworking(_json = json)
        }
    }

    internal class Serializer : BaseSerializer<CredentialNetworking>(CredentialNetworking::class) {
