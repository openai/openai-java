...
private constructor(
    ...
    ) : this(id, code, containerId, outputs, status, type, agent, mutableMapOf())
    ...
    /** A builder for [BetaResponseCodeInterpreterToolCall]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(
            betaResponseCodeInterpreterToolCall: BetaResponseCodeInterpreterToolCall
        ) = apply {
            id = betaResponseCodeInterpreterToolCall.id
            code = betaResponseCodeInterpreterToolCall.code
            containerId = betaResponseCodeInterpreterToolCall.containerId
            outputs = betaResponseCodeInterpreterToolCall.outputs.map { it.toMutableList() }
            status = betaResponseCodeInterpreterToolCall.status
            type = betaResponseCodeInterpreterToolCall.type
            agent = betaResponseCodeInterpreterToolCall.agent
            additionalProperties =
                betaResponseCodeInterpreterToolCall.additionalProperties.toMutableMap()
        }
         ...
         */
        fun build(): BetaResponseCodeInterpreterToolCall =
            BetaResponseCodeInterpreterToolCall(
                checkRequired("id", id),
     ...
     *   expected type.
     */
    fun validate(): BetaResponseCodeInterpreterToolCall = apply {
        if (validated) {
            return@apply
        }
    ...
    private constructor(
         ...
         *     @Override
         *     public Optional<String> visitLogs(Logs logs) {
        ...
        interface Visitor<out T> {
            ...
            /** The image output from the code interpreter. */
            fun visitImage(image: Image): T = unknown(JsonValue.from(image))
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is BetaResponseCodeInterpreterToolCall &&
            id == other.id &&
            code == other.code &&
    ...
    private val hashCode: Int by lazy {
        Objects.hash(id, code, containerId, outputs, status, type, agent, additionalProperties)
    }

    override fun hashCode(): Int = hashCode

    override fun toString() =
        "BetaResponseCodeInterpreterToolCall{id=$id, code=$code, containerId=$containerId, outputs=$outputs, status=$status, type=$type, agent=$agent, additionalProperties=$additionalProperties}"
}
