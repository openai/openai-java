...
@JsonDeserialize(using = InputContentParam.Deserializer::class)
@JsonSerialize(using = InputContentParam.Serializer::class)
class InputContentParam
private constructor(
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = inputContentParam.accept(new InputContentParam.Visitor<Optional<String>>() {
     *     @Override
     ...
     *   expected type.
     */
    fun validate(): InputContentParam = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is InputContentParam &&
    ...
    override fun toString(): String =
        when {
            inputText != null -> "InputContentParam{inputText=$inputText}"
            inputImage != null -> "InputContentParam{inputImage=$inputImage}"
            _json != null -> "InputContentParam{_unknown=$_json}"
            else -> throw IllegalStateException("Invalid InputContentParam")
        }

    companion object {

        /** Text input to the model. */
        @JvmStatic fun ofInputText(inputText: InputText) = InputContentParam(inputText = inputText)

        /** Image input to the model. */
        @JvmStatic
        fun ofInputImage(inputImage: InputImage) = InputContentParam(inputImage = inputImage)
    }

    /**
     * An interface that defines how to map each variant of [InputContentParam] to a value of type
     * [T].
     */
    interface Visitor<out T> {

        /** Text input to the model. */
        fun visitInputText(inputText: InputText): T = unknown(JsonValue.from(inputText))

        /** Image input to the model. */
        fun visitInputImage(inputImage: InputImage): T = unknown(JsonValue.from(inputImage))

        /**
         * Maps an unknown variant of [InputContentParam] to a value of type [T].
         *
         * An instance of [InputContentParam] can contain an unknown variant if it was deserialized
        ...
        fun unknown(json: JsonValue?): T {
            throw OpenAIInvalidDataException("Unknown InputContentParam: $json")
        }
    }

    internal class Deserializer : BaseDeserializer<InputContentParam>(InputContentParam::class) {

        override fun ObjectCodec.deserialize(node: JsonNode): InputContentParam {
            ...
            when (type) {
                "input_text" -> {
                    return tryDeserialize(node, jacksonTypeRef<InputText>())?.let {
                        InputContentParam(inputText = it, _json = json)
                    } ?: InputContentParam(_json = json)
                }
                "input_image" -> {
                    return tryDeserialize(node, jacksonTypeRef<InputImage>())?.let {
                        InputContentParam(inputImage = it, _json = json)
                    } ?: InputContentParam(_json = json)
                }
            }

            return InputContentParam(_json = json)
        }
    }

    internal class Serializer : BaseSerializer<InputContentParam>(InputContentParam::class) {

        override fun serialize(
            value: InputContentParam,
