...
private constructor(
    ...
    /**
     * The unique ID of the item, this can be generated by the client to help manage server-side
     * context, but is not required because the server will generate one if not provided.
     *
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(conversationItem: ConversationItem) = apply {
            id = conversationItem.id
            arguments = conversationItem.arguments
            callId = conversationItem.callId
            content = conversationItem.content.map { it.toMutableList() }
            name = conversationItem.name
            object_ = conversationItem.object_
            output = conversationItem.output
            role = conversationItem.role
            status = conversationItem.status
            type = conversationItem.type
            additionalProperties = conversationItem.additionalProperties.toMutableMap()
        }
    ...
    override fun toString() =
        "ConversationItem{id=$id, arguments=$arguments, callId=$callId, content=$content, name=$name, object_=$object_, output=$output, role=$role, status=$status, type=$type, additionalProperties=$additionalProperties}"
}
