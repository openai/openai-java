...
@JsonDeserialize(using = PersistedAgentTool.Deserializer::class)
@JsonSerialize(using = PersistedAgentTool.Serializer::class)
class PersistedAgentTool
private constructor(
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = persistedAgentTool.accept(new PersistedAgentTool.Visitor<Optional<String>>() {
     *     @Override
     ...
     *   expected type.
     */
    fun validate(): PersistedAgentTool = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is PersistedAgentTool &&
    ...
    override fun toString(): String =
        when {
            function != null -> "PersistedAgentTool{function=$function}"
            toolSearch != null -> "PersistedAgentTool{toolSearch=$toolSearch}"
            programmaticToolCalling != null ->
                "PersistedAgentTool{programmaticToolCalling=$programmaticToolCalling}"
            mcp != null -> "PersistedAgentTool{mcp=$mcp}"
            webSearch != null -> "PersistedAgentTool{webSearch=$webSearch}"
            _json != null -> "PersistedAgentTool{_unknown=$_json}"
            else -> throw IllegalStateException("Invalid PersistedAgentTool")
        }

    companion object {

        /** A function defined by the application. */
        @JvmStatic fun ofFunction(function: Function) = PersistedAgentTool(function = function)

        /** Discovers deferred function tools and loads them into the model context. */
        @JvmStatic
        fun ofToolSearch() =
            PersistedAgentTool(toolSearch = JsonValue.from(mapOf("type" to "tool_search")))
        ...
        /** Tools provided by a remote MCP server without stored credentials. */
        @JvmStatic fun ofMcp(mcp: Mcp) = PersistedAgentTool(mcp = mcp)

        /** Web search. */
        @JvmStatic fun ofWebSearch(webSearch: WebSearch) = PersistedAgentTool(webSearch = webSearch)
    }

    /**
     * An interface that defines how to map each variant of [PersistedAgentTool] to a value of type
     * [T].
     */
    interface Visitor<out T> {
        ...
        /**
         * Maps an unknown variant of [PersistedAgentTool] to a value of type [T].
         *
         * An instance of [PersistedAgentTool] can contain an unknown variant if it was deserialized
        ...
        fun unknown(json: JsonValue?): T {
            throw OpenAIInvalidDataException("Unknown PersistedAgentTool: $json")
        }
    }

    internal class Deserializer : BaseDeserializer<PersistedAgentTool>(PersistedAgentTool::class) {

        override fun ObjectCodec.deserialize(node: JsonNode): PersistedAgentTool {
            ...
            when (type) {
                "function" -> {
                    return tryDeserialize(node, jacksonTypeRef<Function>())?.let {
                        PersistedAgentTool(function = it, _json = json)
                    } ?: PersistedAgentTool(_json = json)
                }
                "tool_search" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                        ?.let { PersistedAgentTool(toolSearch = it, _json = json) }
                        ?.takeIf { it.isValid() } ?: PersistedAgentTool(_json = json)
                }
                "programmatic_tool_calling" -> {
                    return tryDeserialize(node, jacksonTypeRef<ProgrammaticToolCalling>())?.let {
                        PersistedAgentTool(programmaticToolCalling = it, _json = json)
                    } ?: PersistedAgentTool(_json = json)
                }
                "mcp" -> {
                    return tryDeserialize(node, jacksonTypeRef<Mcp>())?.let {
                        PersistedAgentTool(mcp = it, _json = json)
                    } ?: PersistedAgentTool(_json = json)
                }
                "web_search" -> {
                    return tryDeserialize(node, jacksonTypeRef<WebSearch>())?.let {
                        PersistedAgentTool(webSearch = it, _json = json)
                    } ?: PersistedAgentTool(_json = json)
                }
            }

            return PersistedAgentTool(_json = json)
        }
    }

    internal class Serializer : BaseSerializer<PersistedAgentTool>(PersistedAgentTool::class) {

        override fun serialize(
            value: PersistedAgentTool,
    ...
    private constructor(
        ...
        private val transport: JsonField<PersistedMcpTransport>,
        private val type: JsonValue,
        private val additionalProperties: MutableMap<String, JsonValue>,
    ) {
        ...
        private constructor(
            ...
            @ExcludeMissing
            transport: JsonField<PersistedMcpTransport> = JsonMissing.of(),
            @JsonProperty("type") @ExcludeMissing type: JsonValue = JsonMissing.of(),
        ) : this(
            allowedTools,
            connectionOrigin,
         ...
         *   unexpectedly missing or null (e.g. if the server responded with an unexpected value).
         */
        fun transport(): PersistedMcpTransport = transport.getRequired("transport")
        ...
        @JsonProperty("transport")
        @ExcludeMissing
        fun _transport(): JsonField<PersistedMcpTransport> = transport
        ...
        class Builder internal constructor() {
            ...
            private var transport: JsonField<PersistedMcpTransport>? = null
            ...
            /** The credential-free transport used to connect to the MCP server. */
            fun transport(transport: PersistedMcpTransport) = transport(JsonField.of(transport))

            /**
             * Sets [Builder.transport] to an arbitrary JSON value.
             *
             * You should usually call [Builder.transport] with a well-typed [PersistedMcpTransport]
             * value instead. This method is primarily for setting the field to an undocumented or
             * not yet supported value.
             */
            fun transport(transport: JsonField<PersistedMcpTransport>) = apply {
                this.transport = transport
            }

            /** Alias for calling [transport] with `PersistedMcpTransport.ofHttp(http)`. */
            fun transport(http: PersistedMcpTransport.Http) =
                transport(PersistedMcpTransport.ofHttp(http))

            /** Alias for calling [transport] with `PersistedMcpTransport.ofStdio(stdio)`. */
            fun transport(stdio: PersistedMcpTransport.Stdio) =
                transport(PersistedMcpTransport.ofStdio(stdio))
