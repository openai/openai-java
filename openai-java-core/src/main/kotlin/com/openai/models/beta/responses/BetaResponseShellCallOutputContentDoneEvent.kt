...
private constructor(
    ...
    ) : this(commandIndex, itemId, output, outputIndex, sequenceNumber, type, agent, mutableMapOf())
     ...
     * ```java
     * JsonValue.from("response.shell_call_output_content.done")
     * ```
     *
    ...
    /** A builder for [BetaResponseShellCallOutputContentDoneEvent]. */
    class Builder internal constructor() {
        ...
        private var type: JsonValue = JsonValue.from("response.shell_call_output_content.done")
        private var agent: JsonField<Agent> = JsonMissing.of()
        private var additionalProperties: MutableMap<String, JsonValue> = mutableMapOf()

        @JvmSynthetic
        internal fun from(
            betaResponseShellCallOutputContentDoneEvent: BetaResponseShellCallOutputContentDoneEvent
        ) = apply {
            commandIndex = betaResponseShellCallOutputContentDoneEvent.commandIndex
            itemId = betaResponseShellCallOutputContentDoneEvent.itemId
            output = betaResponseShellCallOutputContentDoneEvent.output.map { it.toMutableList() }
            outputIndex = betaResponseShellCallOutputContentDoneEvent.outputIndex
            sequenceNumber = betaResponseShellCallOutputContentDoneEvent.sequenceNumber
            type = betaResponseShellCallOutputContentDoneEvent.type
            agent = betaResponseShellCallOutputContentDoneEvent.agent
            additionalProperties =
                betaResponseShellCallOutputContentDoneEvent.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [BetaResponseShellCallOutputContentDoneEvent].
         *
         ...
         */
        fun build(): BetaResponseShellCallOutputContentDoneEvent =
            BetaResponseShellCallOutputContentDoneEvent(
     ...
     *   expected type.
     */
    fun validate(): BetaResponseShellCallOutputContentDoneEvent = apply {
        if (validated) {
            return@apply
        }
        ...
        sequenceNumber()
        _type().let {
            if (it != JsonValue.from("response.shell_call_output_content.done")) {
    ...
    internal fun validity(): Int =
        (if (commandIndex.asKnown().isPresent) 1 else 0) +
            ...
            type.let {
                if (it == JsonValue.from("response.shell_call_output_content.done")) 1 else 0
            } +
    ...
    private constructor(
        ...
        private constructor(
             ...
             * import java.util.Optional;
             *
             * Optional<String> result = outcome.accept(new Outcome.Visitor<Optional<String>>() {
             *     @Override
             *     public Optional<String> visitTimeout(JsonValue timeout) {
            ...
            interface Visitor<out T> {
                ...
                /** Indicates that the shell call exceeded its configured time limit. */
                fun visitTimeout(timeout: JsonValue): T = unknown(JsonValue.from(timeout))

                /** Indicates that the shell commands finished and returned an exit code. */
                fun visitExit(exit: Exit): T = unknown(JsonValue.from(exit))
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is BetaResponseShellCallOutputContentDoneEvent &&
    ...
    override fun toString() =
        "BetaResponseShellCallOutputContentDoneEvent{commandIndex=$commandIndex, itemId=$itemId, output=$output, outputIndex=$outputIndex, sequenceNumber=$sequenceNumber, type=$type, agent=$agent, additionalProperties=$additionalProperties}"
}
