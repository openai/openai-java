...
private constructor(
     ...
     * ```java
     * JsonValue.from("response.audio_transcript.delta")
     * ```
     *
    ...
    class Builder internal constructor() {
        ...
        private var type: JsonValue = JsonValue.from("response.audio_transcript.delta")
        private var additionalProperties: MutableMap<String, JsonValue> = mutableMapOf()

        @JvmSynthetic
        internal fun from(responseAudioTranscriptDeltaEvent: ResponseAudioTranscriptDeltaEvent) =
            apply {
                contentIndex = responseAudioTranscriptDeltaEvent.contentIndex
                delta = responseAudioTranscriptDeltaEvent.delta
                eventId = responseAudioTranscriptDeltaEvent.eventId
                itemId = responseAudioTranscriptDeltaEvent.itemId
                outputIndex = responseAudioTranscriptDeltaEvent.outputIndex
                responseId = responseAudioTranscriptDeltaEvent.responseId
    ...
    fun validate(): ResponseAudioTranscriptDeltaEvent = apply {
        ...
        responseId()
        _type().let {
            if (it != JsonValue.from("response.audio_transcript.delta")) {
    ...
    internal fun validity(): Int =
        (if (contentIndex.asKnown().isPresent) 1 else 0) +
            ...
            type.let { if (it == JsonValue.from("response.audio_transcript.delta")) 1 else 0 }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is ResponseAudioTranscriptDeltaEvent && contentIndex == other.contentIndex && delta == other.delta && eventId == other.eventId && itemId == other.itemId && outputIndex == other.outputIndex && responseId == other.responseId && type == other.type && additionalProperties == other.additionalProperties /* spotless:on */
    }

    /* spotless:off */
    private val hashCode: Int by lazy { Objects.hash(contentIndex, delta, eventId, itemId, outputIndex, responseId, type, additionalProperties) }
    /* spotless:on */

    override fun hashCode(): Int = hashCode

    override fun toString() =
        "ResponseAudioTranscriptDeltaEvent{contentIndex=$contentIndex, delta=$delta, eventId=$eventId, itemId=$itemId, outputIndex=$outputIndex, responseId=$responseId, type=$type, additionalProperties=$additionalProperties}"
}
