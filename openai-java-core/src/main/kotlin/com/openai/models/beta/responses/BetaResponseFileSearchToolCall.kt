...
private constructor(
    ...
    ) : this(id, queries, status, type, agent, results, mutableMapOf())
    ...
    /** A builder for [BetaResponseFileSearchToolCall]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(betaResponseFileSearchToolCall: BetaResponseFileSearchToolCall) = apply {
            id = betaResponseFileSearchToolCall.id
            queries = betaResponseFileSearchToolCall.queries.map { it.toMutableList() }
            status = betaResponseFileSearchToolCall.status
            type = betaResponseFileSearchToolCall.type
            agent = betaResponseFileSearchToolCall.agent
            results = betaResponseFileSearchToolCall.results.map { it.toMutableList() }
            additionalProperties =
                betaResponseFileSearchToolCall.additionalProperties.toMutableMap()
        }
         ...
         */
        fun build(): BetaResponseFileSearchToolCall =
            BetaResponseFileSearchToolCall(
                checkRequired("id", id),
     ...
     *   expected type.
     */
    fun validate(): BetaResponseFileSearchToolCall = apply {
        if (validated) {
            return@apply
        }
    ...
    private constructor(
        ...
        private val hashCode: Int by lazy {
            Objects.hash(attributes, fileId, filename, score, text, additionalProperties)
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is BetaResponseFileSearchToolCall &&
            id == other.id &&
    ...
    private val hashCode: Int by lazy {
        Objects.hash(id, queries, status, type, agent, results, additionalProperties)
    }

    override fun hashCode(): Int = hashCode

    override fun toString() =
        "BetaResponseFileSearchToolCall{id=$id, queries=$queries, status=$status, type=$type, agent=$agent, results=$results, additionalProperties=$additionalProperties}"
}
