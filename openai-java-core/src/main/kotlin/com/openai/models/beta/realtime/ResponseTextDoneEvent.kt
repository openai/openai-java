...
private constructor(
    ...
    ) : this(contentIndex, eventId, itemId, outputIndex, responseId, text, type, mutableMapOf())
    ...
    class Builder internal constructor() {
        ...
        private var type: JsonValue = JsonValue.from("response.text.done")
        ...
        internal fun from(responseTextDoneEvent: ResponseTextDoneEvent) = apply {
            ...
            eventId = responseTextDoneEvent.eventId
            itemId = responseTextDoneEvent.itemId
            outputIndex = responseTextDoneEvent.outputIndex
            responseId = responseTextDoneEvent.responseId
    ...
    fun validate(): ResponseTextDoneEvent = apply {
        ...
        text()
        _type().let {
            if (it != JsonValue.from("response.text.done")) {
    ...
    internal fun validity(): Int =
        (if (contentIndex.asKnown().isPresent) 1 else 0) +
            ...
            type.let { if (it == JsonValue.from("response.text.done")) 1 else 0 }
    ...
    override fun toString() =
        "ResponseTextDoneEvent{contentIndex=$contentIndex, eventId=$eventId, itemId=$itemId, outputIndex=$outputIndex, responseId=$responseId, text=$text, type=$type, additionalProperties=$additionalProperties}"
}
