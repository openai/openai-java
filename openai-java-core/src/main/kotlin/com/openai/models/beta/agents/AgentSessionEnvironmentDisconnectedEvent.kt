...
private constructor(
     ...
     * ```java
     * JsonValue.from("agent.session.environment.disconnected")
     * ```
     *
    ...
    /** A builder for [AgentSessionEnvironmentDisconnectedEvent]. */
    class Builder internal constructor() {
        ...
        private var type: JsonValue = JsonValue.from("agent.session.environment.disconnected")
        private var additionalProperties: MutableMap<String, JsonValue> = mutableMapOf()

        @JvmSynthetic
        internal fun from(
            agentSessionEnvironmentDisconnectedEvent: AgentSessionEnvironmentDisconnectedEvent
        ) = apply {
            environment = agentSessionEnvironmentDisconnectedEvent.environment
            eventId = agentSessionEnvironmentDisconnectedEvent.eventId
            sessionId = agentSessionEnvironmentDisconnectedEvent.sessionId
            turnId = agentSessionEnvironmentDisconnectedEvent.turnId
            type = agentSessionEnvironmentDisconnectedEvent.type
            additionalProperties =
                agentSessionEnvironmentDisconnectedEvent.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [AgentSessionEnvironmentDisconnectedEvent].
         *
         ...
         */
        fun build(): AgentSessionEnvironmentDisconnectedEvent =
            AgentSessionEnvironmentDisconnectedEvent(
     ...
     *   expected type.
     */
    fun validate(): AgentSessionEnvironmentDisconnectedEvent = apply {
        if (validated) {
            return@apply
        }
        ...
        turnId()
        _type().let {
            if (it != JsonValue.from("agent.session.environment.disconnected")) {
    ...
    internal fun validity(): Int =
        (environment.asKnown().getOrNull()?.validity() ?: 0) +
            ...
            type.let {
                if (it == JsonValue.from("agent.session.environment.disconnected")) 1 else 0
            }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is AgentSessionEnvironmentDisconnectedEvent &&
    ...
    override fun toString() =
        "AgentSessionEnvironmentDisconnectedEvent{environment=$environment, eventId=$eventId, sessionId=$sessionId, turnId=$turnId, type=$type, additionalProperties=$additionalProperties}"
}
