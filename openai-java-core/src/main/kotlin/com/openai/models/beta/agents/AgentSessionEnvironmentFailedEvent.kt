...
private constructor(
     ...
     * ```java
     * JsonValue.from("agent.session.environment.failed")
     * ```
     *
    ...
    /** A builder for [AgentSessionEnvironmentFailedEvent]. */
    class Builder internal constructor() {
        ...
        private var type: JsonValue = JsonValue.from("agent.session.environment.failed")
        private var additionalProperties: MutableMap<String, JsonValue> = mutableMapOf()

        @JvmSynthetic
        internal fun from(agentSessionEnvironmentFailedEvent: AgentSessionEnvironmentFailedEvent) =
            apply {
                environment = agentSessionEnvironmentFailedEvent.environment
                eventId = agentSessionEnvironmentFailedEvent.eventId
                sessionId = agentSessionEnvironmentFailedEvent.sessionId
                turnId = agentSessionEnvironmentFailedEvent.turnId
                type = agentSessionEnvironmentFailedEvent.type
                additionalProperties =
                    agentSessionEnvironmentFailedEvent.additionalProperties.toMutableMap()
            }
        ...
        /**
         * Returns an immutable instance of [AgentSessionEnvironmentFailedEvent].
         *
         ...
         */
        fun build(): AgentSessionEnvironmentFailedEvent =
            AgentSessionEnvironmentFailedEvent(
     ...
     *   expected type.
     */
    fun validate(): AgentSessionEnvironmentFailedEvent = apply {
        if (validated) {
            return@apply
        }
        ...
        turnId()
        _type().let {
            if (it != JsonValue.from("agent.session.environment.failed")) {
    ...
    internal fun validity(): Int =
        (environment.asKnown().getOrNull()?.validity() ?: 0) +
            ...
            type.let { if (it == JsonValue.from("agent.session.environment.failed")) 1 else 0 }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is AgentSessionEnvironmentFailedEvent &&
    ...
    override fun toString() =
        "AgentSessionEnvironmentFailedEvent{environment=$environment, eventId=$eventId, sessionId=$sessionId, turnId=$turnId, type=$type, additionalProperties=$additionalProperties}"
}
