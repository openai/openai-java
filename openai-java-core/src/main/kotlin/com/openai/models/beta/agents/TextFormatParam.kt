...
@JsonDeserialize(using = TextFormatParam.Deserializer::class)
@JsonSerialize(using = TextFormatParam.Serializer::class)
class TextFormatParam
private constructor(
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = textFormatParam.accept(new TextFormatParam.Visitor<Optional<String>>() {
     *     @Override
     ...
     *   expected type.
     */
    fun validate(): TextFormatParam = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is TextFormatParam && text == other.text && jsonSchema == other.jsonSchema
    }
    ...
    override fun toString(): String =
        when {
            text != null -> "TextFormatParam{text=$text}"
            jsonSchema != null -> "TextFormatParam{jsonSchema=$jsonSchema}"
            _json != null -> "TextFormatParam{_unknown=$_json}"
            else -> throw IllegalStateException("Invalid TextFormatParam")
        }

    companion object {

        /** Generates ordinary text without a structured-output constraint. */
        @JvmStatic fun ofText() = TextFormatParam(text = JsonValue.from(mapOf("type" to "text")))

        /** Constrains generated text to a JSON Schema. */
        @JvmStatic
        fun ofJsonSchema(jsonSchema: JsonSchema) = TextFormatParam(jsonSchema = jsonSchema)
    }

    /**
     * An interface that defines how to map each variant of [TextFormatParam] to a value of type
     * [T].
     */
    interface Visitor<out T> {
        ...
        /**
         * Maps an unknown variant of [TextFormatParam] to a value of type [T].
         *
         * An instance of [TextFormatParam] can contain an unknown variant if it was deserialized
    ...
    internal class Deserializer : BaseDeserializer<TextFormatParam>(TextFormatParam::class) {

        override fun ObjectCodec.deserialize(node: JsonNode): TextFormatParam {
            ...
            when (type) {
                "text" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonValue>())
                        ?.let { TextFormatParam(text = it, _json = json) }
                        ?.takeIf { it.isValid() } ?: TextFormatParam(_json = json)
                }
                "json_schema" -> {
                    return tryDeserialize(node, jacksonTypeRef<JsonSchema>())?.let {
                        TextFormatParam(jsonSchema = it, _json = json)
                    } ?: TextFormatParam(_json = json)
                }
            }

            return TextFormatParam(_json = json)
        }
    }

    internal class Serializer : BaseSerializer<TextFormatParam>(TextFormatParam::class) {

        override fun serialize(
            value: TextFormatParam,
