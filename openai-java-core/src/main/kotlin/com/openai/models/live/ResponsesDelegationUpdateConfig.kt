...
/**
 * Updates to the Responses backend of an existing Live session. Omitted settings retain their
 * current values.
 */
...
private constructor(
    ...
    /**
     * The Responses backend model to use for subsequent delegated requests. Omit to keep the
    ...
    /** A builder for [ResponsesDelegationUpdateConfig]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(responsesDelegationUpdateConfig: ResponsesDelegationUpdateConfig) =
            apply {
                instructions = responsesDelegationUpdateConfig.instructions
                maxOutputTokens = responsesDelegationUpdateConfig.maxOutputTokens
                model = responsesDelegationUpdateConfig.model
                parallelToolCalls = responsesDelegationUpdateConfig.parallelToolCalls
                reasoning = responsesDelegationUpdateConfig.reasoning
                serviceTier = responsesDelegationUpdateConfig.serviceTier
                text = responsesDelegationUpdateConfig.text
                toolChoice = responsesDelegationUpdateConfig.toolChoice
                tools = responsesDelegationUpdateConfig.tools.map { it.toMutableList() }
                additionalProperties =
                    responsesDelegationUpdateConfig.additionalProperties.toMutableMap()
            }
        ...
        /**
         * Returns an immutable instance of [ResponsesDelegationUpdateConfig].
         *
         * Further updates to this [Builder] will not mutate the returned instance.
         */
        fun build(): ResponsesDelegationUpdateConfig =
            ResponsesDelegationUpdateConfig(
                instructions,
                maxOutputTokens,
                model,
     ...
     *   expected type.
     */
    fun validate(): ResponsesDelegationUpdateConfig = apply {
        if (validated) {
            return@apply
        }
    ...
    private constructor(
        ...
        interface Visitor<out T> {
            ...
            fun visitLiveToolChoiceEnum(liveToolChoiceEnum: LiveToolChoiceEnum): T =
                unknown(JsonValue.from(liveToolChoiceEnum))

            fun visitLiveFunctionToolChoiceParam(
                liveFunctionToolChoiceParam: LiveFunctionToolChoiceParam
            ): T = unknown(JsonValue.from(liveFunctionToolChoiceParam))

            fun visitLiveMcpToolChoiceParam(liveMcpToolChoiceParam: LiveMcpToolChoiceParam): T =
                unknown(JsonValue.from(liveMcpToolChoiceParam))
    ...
    private constructor(
        ...
        interface Visitor<out T> {
            ...
            /** A web search tool available to the Live session’s Responses backend. */
            fun visitWebSearch(webSearch: JsonValue): T = unknown(JsonValue.from(webSearch))
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is ResponsesDelegationUpdateConfig &&
    ...
    override fun toString() =
        "ResponsesDelegationUpdateConfig{instructions=$instructions, maxOutputTokens=$maxOutputTokens, model=$model, parallelToolCalls=$parallelToolCalls, reasoning=$reasoning, serviceTier=$serviceTier, text=$text, toolChoice=$toolChoice, tools=$tools, additionalProperties=$additionalProperties}"
}
