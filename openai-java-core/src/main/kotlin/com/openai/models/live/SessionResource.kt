...
private constructor(
    ...
    /**
     * The unique ID of the Live session. Use this ID for sideband connections, forking, and
     * recording download.
     *
    ...
    /**
     * The status of the session snapshot. Always `active`, including the final snapshot in
     * session.closed; use the event type to determine that the session has closed.
     *
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [SessionResource].
         *
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(sessionResource: SessionResource) = apply {
            id = sessionResource.id
            expiresAt = sessionResource.expiresAt
            model = sessionResource.model
            status = sessionResource.status
            audio = sessionResource.audio
            client = sessionResource.client
            delegation = sessionResource.delegation
            input = sessionResource.input.map { it.toMutableList() }
            instructions = sessionResource.instructions
            store = sessionResource.store
            additionalProperties = sessionResource.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [SessionResource].
         *
     ...
     *   expected type.
     */
    fun validate(): SessionResource = apply {
        if (validated) {
            return@apply
        }
        ...
        _status().let {
            if (it != JsonValue.from("active")) {
                throw OpenAIInvalidDataException("'status' is invalid, received $it")
            }
        }
        ...
        private constructor(
            ...
            private constructor(
                ...
                interface Visitor<out T> {
                    ...
                    /** A built-in voice available for Live speech. */
                    fun visitBuiltIn(builtIn: BuiltInVoice): T = unknown(JsonValue.from(builtIn))

                    fun visitCustom(custom: CustomVoice): T = unknown(JsonValue.from(custom))
    ...
    private constructor(
        ...
        interface Visitor<out T> {
             ...
             * your backend handles.
             */
            fun visitClient(client: ClientDelegation): T = unknown(JsonValue.from(client))
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is SessionResource &&
            id == other.id &&
    ...
    override fun toString() =
        "SessionResource{id=$id, expiresAt=$expiresAt, model=$model, status=$status, audio=$audio, client=$client, delegation=$delegation, input=$input, instructions=$instructions, store=$store, additionalProperties=$additionalProperties}"
}
