...
/**
 * Client events for a Live fork WebSocket. First send session.start with an overrides object (which
 * may be empty), then wait for session.started before sending other commands. The model and
 * conversation are inherited from the stored session.
 */
@JsonDeserialize(using = ForkClientEvent.Deserializer::class)
@JsonSerialize(using = ForkClientEvent.Serializer::class)
class ForkClientEvent
private constructor(
    private val sessionStart: ForkSessionStartEvent? = null,
    ...
    /**
     * Start a Live session after connecting to a stored session’s fork WebSocket. Send an empty
     * `session` object to use the stored configuration.
     */
    fun sessionStart(): Optional<ForkSessionStartEvent> = Optional.ofNullable(sessionStart)
     ...
     * `session` object to use the stored configuration.
     */
    fun asSessionStart(): ForkSessionStartEvent = sessionStart.getOrThrow("sessionStart")
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = forkClientEvent.accept(new ForkClientEvent.Visitor<Optional<String>>() {
     *     @Override
     *     public Optional<String> visitSessionStart(ForkSessionStartEvent sessionStart) {
     ...
     *   expected type.
     */
    fun validate(): ForkClientEvent = apply {
        if (validated) {
            return@apply
        }
        ...
        accept(
            object : Visitor<Unit> {
                override fun visitSessionStart(sessionStart: ForkSessionStartEvent) {
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is ForkClientEvent &&
    ...
    override fun toString(): String =
        when {
            sessionStart != null -> "ForkClientEvent{sessionStart=$sessionStart}"
            sessionUpdate != null -> "ForkClientEvent{sessionUpdate=$sessionUpdate}"
            sessionInputAudioAppend != null ->
                "ForkClientEvent{sessionInputAudioAppend=$sessionInputAudioAppend}"
            sessionInputAudioMute != null ->
                "ForkClientEvent{sessionInputAudioMute=$sessionInputAudioMute}"
            sessionInputAudioUnmute != null ->
                "ForkClientEvent{sessionInputAudioUnmute=$sessionInputAudioUnmute}"
            sessionInstructionsAppend != null ->
                "ForkClientEvent{sessionInstructionsAppend=$sessionInstructionsAppend}"
            sessionThinkingAppend != null ->
                "ForkClientEvent{sessionThinkingAppend=$sessionThinkingAppend}"
            sessionCommentaryAppend != null ->
                "ForkClientEvent{sessionCommentaryAppend=$sessionCommentaryAppend}"
            responseItemCreate != null -> "ForkClientEvent{responseItemCreate=$responseItemCreate}"
            responseCreate != null -> "ForkClientEvent{responseCreate=$responseCreate}"
            sessionClose != null -> "ForkClientEvent{sessionClose=$sessionClose}"
            _json != null -> "ForkClientEvent{_unknown=$_json}"
            else -> throw IllegalStateException("Invalid ForkClientEvent")
        }
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun ofSessionStart(sessionStart: ForkSessionStartEvent) =
            ForkClientEvent(sessionStart = sessionStart)
        ...
        fun ofSessionUpdate(sessionUpdate: SessionUpdateEvent) =
            ForkClientEvent(sessionUpdate = sessionUpdate)
        ...
        fun ofResponseItemCreate(responseItemCreate: ResponseItemCreateEvent) =
            ForkClientEvent(responseItemCreate = responseItemCreate)
        ...
        fun ofResponseCreate(responseCreate: ResponseCreateEvent) =
            ForkClientEvent(responseCreate = responseCreate)
        ...
        fun ofSessionClose(sessionClose: SessionCloseEvent) =
            ForkClientEvent(sessionClose = sessionClose)
    }

    /**
     * An interface that defines how to map each variant of [ForkClientEvent] to a value of type
     * [T].
     */
    interface Visitor<out T> {
         ...
         * `session` object to use the stored configuration.
         */
        fun visitSessionStart(sessionStart: ForkSessionStartEvent): T
        ...
        /**
         * Maps an unknown variant of [ForkClientEvent] to a value of type [T].
         *
         * An instance of [ForkClientEvent] can contain an unknown variant if it was deserialized
    ...
    internal class Deserializer : BaseDeserializer<ForkClientEvent>(ForkClientEvent::class) {

        override fun ObjectCodec.deserialize(node: JsonNode): ForkClientEvent {
            val json = JsonValue.fromJsonNode(node)
            val type = json.asObject().getOrNull()?.get("type")?.asString()?.getOrNull()

            when (type) {
                "session.start" -> {
                    return tryDeserialize(node, jacksonTypeRef<ForkSessionStartEvent>())?.let {
                        ForkClientEvent(sessionStart = it, _json = json)
                    } ?: ForkClientEvent(_json = json)
                }
                "session.update" -> {
                    return tryDeserialize(node, jacksonTypeRef<SessionUpdateEvent>())?.let {
                        ForkClientEvent(sessionUpdate = it, _json = json)
                    } ?: ForkClientEvent(_json = json)
                }
                "session.input_audio.append" -> {
                    return tryDeserialize(node, jacksonTypeRef<InputAudioAppendEvent>())?.let {
                        ForkClientEvent(sessionInputAudioAppend = it, _json = json)
                    } ?: ForkClientEvent(_json = json)
                }
                "session.input_audio.mute" -> {
                    return tryDeserialize(node, jacksonTypeRef<InputAudioMuteEvent>())?.let {
                        ForkClientEvent(sessionInputAudioMute = it, _json = json)
                    } ?: ForkClientEvent(_json = json)
                }
                "session.input_audio.unmute" -> {
                    return tryDeserialize(node, jacksonTypeRef<InputAudioUnmuteEvent>())?.let {
                        ForkClientEvent(sessionInputAudioUnmute = it, _json = json)
                    } ?: ForkClientEvent(_json = json)
                }
                "session.instructions.append" -> {
                    return tryDeserialize(node, jacksonTypeRef<InstructionsAppendEvent>())?.let {
                        ForkClientEvent(sessionInstructionsAppend = it, _json = json)
                    } ?: ForkClientEvent(_json = json)
                }
                "session.thinking.append" -> {
                    return tryDeserialize(node, jacksonTypeRef<ThinkingAppendEvent>())?.let {
                        ForkClientEvent(sessionThinkingAppend = it, _json = json)
                    } ?: ForkClientEvent(_json = json)
                }
                "session.commentary.append" -> {
                    return tryDeserialize(node, jacksonTypeRef<CommentaryAppendEvent>())?.let {
                        ForkClientEvent(sessionCommentaryAppend = it, _json = json)
                    } ?: ForkClientEvent(_json = json)
                }
                "response.item.create" -> {
                    return tryDeserialize(node, jacksonTypeRef<ResponseItemCreateEvent>())?.let {
                        ForkClientEvent(responseItemCreate = it, _json = json)
                    } ?: ForkClientEvent(_json = json)
                }
                "response.create" -> {
                    return tryDeserialize(node, jacksonTypeRef<ResponseCreateEvent>())?.let {
                        ForkClientEvent(responseCreate = it, _json = json)
                    } ?: ForkClientEvent(_json = json)
                }
                "session.close" -> {
                    return tryDeserialize(node, jacksonTypeRef<SessionCloseEvent>())?.let {
                        ForkClientEvent(sessionClose = it, _json = json)
                    } ?: ForkClientEvent(_json = json)
                }
            }

            return ForkClientEvent(_json = json)
        }
    }

    internal class Serializer : BaseSerializer<ForkClientEvent>(ForkClientEvent::class) {

        override fun serialize(
            value: ForkClientEvent,
