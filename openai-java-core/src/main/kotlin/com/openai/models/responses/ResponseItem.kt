...
private constructor(
    ...
    private val customToolCall: ResponseCustomToolCallItem? = null,
    ...
    fun customToolCall(): Optional<ResponseCustomToolCallItem> = Optional.ofNullable(customToolCall)
    ...
    /** A call to a custom tool created by the model. */
    fun asCustomToolCall(): ResponseCustomToolCallItem = customToolCall.getOrThrow("customToolCall")
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = responseItem.accept(new ResponseItem.Visitor<Optional<String>>() {
     *     @Override
     *     public Optional<String> visitResponseInputMessageItem(ResponseInputMessageItem responseInputMessageItem) {
     *         return Optional.of(responseInputMessageItem.toString());
     *     }
     *
     *     // ...
     *
    ...
    fun validate(): ResponseItem = apply {
        ...
        accept(
            object : Visitor<Unit> {
                ...
                override fun visitCustomToolCall(customToolCall: ResponseCustomToolCallItem) {
    ...
    override fun toString(): String =
        when {
            ...
            compaction != null -> "ResponseItem{compaction=$compaction}"
            ...
            shellCall != null -> "ResponseItem{shellCall=$shellCall}"
            shellCallOutput != null -> "ResponseItem{shellCallOutput=$shellCallOutput}"
            applyPatchCall != null -> "ResponseItem{applyPatchCall=$applyPatchCall}"
            applyPatchCallOutput != null ->
                "ResponseItem{applyPatchCallOutput=$applyPatchCallOutput}"
            ...
            customToolCall != null -> "ResponseItem{customToolCall=$customToolCall}"
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun ofReasoning(reasoning: ResponseReasoningItem) = ResponseItem(reasoning = reasoning)

        @JvmStatic fun ofProgram(program: Program) = ResponseItem(program = program)

        @JvmStatic
        fun ofProgramOutput(programOutput: ProgramOutput) =
            ResponseItem(programOutput = programOutput)
         ...
         */
        @JvmStatic
        fun ofCompaction(compaction: ResponseCompactionItem) = ResponseItem(compaction = compaction)
        ...
        fun ofApplyPatchCall(applyPatchCall: ResponseApplyPatchToolCall) =
            ResponseItem(applyPatchCall = applyPatchCall)
        ...
        /** A call to a custom tool created by the model. */
        @JvmStatic
        fun ofCustomToolCall(customToolCall: ResponseCustomToolCallItem) =
    ...
    interface Visitor<out T> {
        ...
        /** A call to a custom tool created by the model. */
        fun visitCustomToolCall(customToolCall: ResponseCustomToolCallItem): T
            ...
            when (type) {
                ...
                "shell_call" -> {
                    return tryDeserialize(node, jacksonTypeRef<ResponseFunctionShellToolCall>())
                        ?.let { ResponseItem(shellCall = it, _json = json) }
                        ?: ResponseItem(_json = json)
                }
                "shell_call_output" -> {
                    return tryDeserialize(
                ...
                "custom_tool_call" -> {
                    return tryDeserialize(node, jacksonTypeRef<ResponseCustomToolCallItem>())?.let {
                        ResponseItem(customToolCall = it, _json = json)
                    } ?: ResponseItem(_json = json)
                }
                "custom_tool_call_output" -> {
                    return tryDeserialize(node, jacksonTypeRef<ResponseCustomToolCallOutputItem>())
                        ?.let { ResponseItem(customToolCallOutput = it, _json = json) }
                        ?: ResponseItem(_json = json)
                }
            }
    ...
    private constructor(
        ...
        override fun toString() =
            "ImageGenerationCall{id=$id, result=$result, status=$status, type=$type, action=$action, background=$background, outputFormat=$outputFormat, quality=$quality, revisedPrompt=$revisedPrompt, size=$size, additionalProperties=$additionalProperties}"
    }
    ...
    private constructor(
        ...
        override fun toString() =
            "LocalShellCallOutput{id=$id, callId=$callId, output=$output, type=$type, status=$status, additionalProperties=$additionalProperties}"
    }
