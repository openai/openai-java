...
private constructor(
    ...
    init {
        ...
        // When this future is completed externally, clean up the cancellation token
        this.whenComplete { _, _ -> cancellationTokenSource.close() }
    }
    ...
    companion object {
        ...
        /**
         * Creates a [CancellableCompletableFuture] that is already completed with the given value.
         */
        @JvmStatic
        fun <T> completedFuture(value: T): CancellableCompletableFuture<T> {
            val future = create<T>()
        ...
        /**
         * Creates a [CancellableCompletableFuture] that is already completed exceptionally.
         */
        @JvmStatic
        fun <T> failedFuture(error: Throwable): CancellableCompletableFuture<T> {
            val future = create<T>()
