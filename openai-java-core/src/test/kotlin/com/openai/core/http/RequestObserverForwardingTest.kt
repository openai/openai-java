...
internal class RequestObserverForwardingTest {
    ...
    @Test
    fun requestDefaultsPreserveCallObserverAndCallerTimeout() {
        ...
        val callOptions =
            RequestOptions.builder()
                ...
                .build()
        val defaults = RequestOptions.builder().timeout(Duration.ofSeconds(10)).build()

        val merged = callOptions.applyDefaults(defaults)

        assertThat(merged.requestObserver).isSameAs(observer)
        assertThat(merged.timeout).isNotNull()
    }
    ...
    private fun assertForwarding(
        ...
        assertThat(transport.options).hasSize(2).allSatisfy {
            assertThat(it.requestObserver).isSameAs(observer)
        }
    ...
    private fun execute(
        ...
        if (async) client.executeAsync(request, requestOptions.withRequestObserver(observer)).join()
        else client.execute(request, requestOptions.withRequestObserver(observer))
    ...
    private class RecordingTransport(vararg statuses: Int) : HttpClient {
        ...
        override fun execute(request: HttpRequest, requestOptions: RequestOptions): HttpResponse =
            record(request, requestOptions)

        override fun executeAsync(
            request: HttpRequest,
            requestOptions: RequestOptions,
        ): CompletableFuture<HttpResponse> =
            CompletableFuture.completedFuture(record(request, requestOptions))

        private fun record(request: HttpRequest, requestOptions: RequestOptions): HttpResponse {
            requests.add(request)
            options.add(requestOptions)
            requestOptions.requestObserver?.let { observer ->
