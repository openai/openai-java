// Curated from CredentialAuthRotateParamTest at 59d019d42daf06d3369c76b7f98a00ee2c7c3d2f.
// Compile-only coverage for the unchanged Agents beta credential API. The three expressions
// chaining through Body.auth() intentionally use the original auth value instead: credential
// updates now allow metadata without auth, changing that getter to Optional.
// Keep the ordinary test in src/test for runtime assertions, including secret redaction.
...
internal class CredentialAuthRotateCompatibility {
    ...
    fun environmentVariableStringsRedactAdditionalSecretWithoutChangingSerialization(
        ...
        assertThat(auth.asEnvironmentVariable()).isEqualTo(environmentVariable)
    ...
    fun environmentVariableStringsRedactSecretWithoutChangingSerialization() {
        ...
        assertThat(mapper.readTree(mapper.writeValueAsString(auth)).get("secret_value").asText())
            .isEqualTo(secret)
        assertThat(auth.asEnvironmentVariable().secretValue()).isEqualTo(secret)
    }
