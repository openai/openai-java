...
The standard client uses the existing Java 8 compatible OkHttp transport for HTTP and WebSocket connections. All connections disable extension negotiation and reject upgrade responses containing extensions before the native reader starts. This prevents compressed-message amplification, including with the default receive limit. OkHttp still assembles each complete message before the SDK checks its size and delivers it, so `maxMessageBytes` is not a memory-allocation bound: a single oversized uncompressed message can allocate memory before rejection. The default (`Integer.MAX_VALUE`) continues to accept large uncompressed messages. Queued-event, queued-byte and cumulative response limits remain enforced.
